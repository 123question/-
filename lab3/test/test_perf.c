...
int main(void) {
    ...
    /* CTR with T-table backend */
    sm4_ctr_ttable_init(&ctx, key, iv, 0);
    start = get_time();
    sm4_ctr_ttable_encrypt(&ctx, plain, cipher, DATA_SIZE);
    end = get_time();
    ctr_ttable_time = end - start;
    print_result("CTR T-table Mode", ctr_ttable_time, DATA_SIZE);
    printf("  Speedup vs CTR: %.2fx\n", ctr_time / ctr_ttable_time);
