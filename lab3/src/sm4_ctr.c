...
void sm4_ctr_ttable_init(SM4_Ctx *ctx, const uint8_t *key,
                         const uint8_t *iv, uint64_t counter) {
...
static void sm4_ctr_ttable_crypt(SM4_Ctx *ctx, const uint8_t *in,
                                 uint8_t *out, size_t length) {
    uint8_t counter_block[SM4_BLOCK_SIZE];
    uint8_t keystream[SM4_BLOCK_SIZE];
    size_t processed = 0;
    ...
    while (processed < length) {
        ...
        sm4_ttable_encrypt_block(ctx, counter_block, keystream);

        size_t remaining = length - processed;
        size_t chunk = (remaining < SM4_BLOCK_SIZE) ?
...
void sm4_ctr_ttable_encrypt(SM4_Ctx *ctx, const uint8_t *in,
                            uint8_t *out, size_t length) {
    sm4_ctr_ttable_crypt(ctx, in, out, length);
}

void sm4_ctr_ttable_decrypt(SM4_Ctx *ctx, const uint8_t *in,
                            uint8_t *out, size_t length) {
    sm4_ctr_ttable_crypt(ctx, in, out, length);
}