...
\author{
    \textbf{网安2班} 李瑞佳~~202300460078 \\
    \textbf{网安2班} 岳智宇~~202300460074 \\
    \textbf{网安2班} 张金浩~~202300460059 \\
    \textbf{密码1班} 薛俊东~~202300460087 \\
}
\date{\today}
...
\fancyhead[L]{
    \begin{minipage}[c]{0.9\textwidth}
        \includegraphics[height=10.5mm]{logo1.jpg}
    \end{minipage}
}
\fancyhead[C]{作业七：基于garak的大语言模型安全测评}
\fancyfoot[C]{\thepage}

\begin{titlingpage}
\centering
\includegraphics[width=0.8\textwidth]{logo2.jpg}

\vspace{2cm}

{\Huge \bfseries 作业七：基于garak的大语言模型安全测评\par}
...
本测评共覆盖11个安全类别，对齐 OWASP LLM Top 10 风险分类：
...
\begin{enumerate}[label=(\arabic*)]
    \item \textbf{环境变量命名：} 环境变量名应为OPENAICOMPATIBLE\_API\_KEY
    \item \textbf{YAML 键名规则：} generator 键名需使用模块名而非类名，garak 通过模块属性加载配置命名空间
    \item \textbf{编码问题：} Windows 终端输出中文存在乱码，需设置PYTHONIOENCODING=utf-8
